  ...
  return (
    <div style={overlayStyle} onClick={onClose}>
      <div style={panelStyle} className="rw-modal-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <h2 style={titleStyle} className="rw-modal-title">{locationText}</h2>
        <div style={dateRow}>
          <CalendarIcon />
          ...
          <div>
            ...
            <div style={{ ...supportCard, marginTop: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                <HeartIcon />
                <span style={{ fontSize: 14, fontWeight: 700, color: '#A61C24' }}>Dukungan Warga</span>
              </div>
                ...
                <div style={{ display: 'flex' }}>
                  {supporters.slice(0, 3).map((s, i) => (
                    <div key={s.id} style={{ marginLeft: i === 0 ? 0 : -10 }}>
                      {s.avatar_url ? (
                        <img src={s.avatar_url} alt={s.username || 'Pendukung'} style={supporterAvatarImg} />
                      ) : (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <ChatIcon />
                <span style={{ fontSize: 14, fontWeight: 700, color: '#A61C24' }}>Komentar Terbaru</span>
              </div>
              ...
              {!commentsLoading && comments.slice(-5).reverse().map((c, i) => {
...
const closeBtn = { border: 'none', background: '#f1f3f5', width: 36, height: 36, borderRadius: '50%', cursor: 'pointer', fontSize: 14, color: '#495057', flexShrink: 0 };
const titleStyle = { fontSize: 24, fontWeight: 700, margin: '0 0 8px' };
const dateRow = { display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#868e96', marginBottom: 20 };
...
const photoStyle = { width: '100%', height: 180, objectFit: 'cover', borderRadius: 10, display: 'block' };
const placeholderBox = { background: '#f8f9fa', border: '1px dashed #dee2e6', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#adb5bd', fontSize: 12, textAlign: 'center', padding: 12 };

const sectionLabel = { display: 'flex', alignItems: 'center', gap: 6, fontSize: 14, fontWeight: 700, color: '#212529', marginBottom: 10 };
const descBox = { background: '#f8f9fa', borderRadius: 10, padding: '12px 14px', fontSize: 13.5, color: '#495057', lineHeight: 1.5 };
const infoPill = { fontSize: 12, fontWeight: 600, color: '#A61C24', background: '#FDECEE', padding: '5px 12px', borderRadius: 999 };

const statusCard = { background: '#fff', border: '1px solid #f1f3f5', borderRadius: 12, padding: 16 };
const supportCard = { background: '#FDECEE', border: '1px solid #f6c9ce', borderRadius: 12, padding: 16 };
...
const commentAvatarImg = { width: 20, height: 20, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 };
const commentText = { fontSize: 12, color: '#A61C24', fontWeight: 500 };

const supporterAvatarImg = { width: 28, height: 28, borderRadius: '50%', objectFit: 'cover', border: '2px solid #FDECEE', display: 'block' };
const supporterAvatarFallback = { background: '#e08e8e', color: '#fff', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' };