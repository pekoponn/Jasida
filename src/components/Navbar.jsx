...
export default function Navbar() {
  ...
  return (
    ...
    <nav className="rw-mobile-bottom-nav">
        {isAdmin ? (
          <>
            <NavLink to="/admin" end className={({ isActive }) => `rw-bottom-item ${isActive ? 'active' : ''}`}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z" />
              </svg>
              <span>Laporan</span>
            </NavLink>
            <NavLink to="/admin/peta" className={({ isActive }) => `rw-bottom-item ${isActive ? 'active' : ''}`}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
          ...
          <>
            <NavLink to="/" end className={({ isActive }) => `rw-bottom-item ${isActive ? 'active' : ''}`}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
                <path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z" />
              </svg>
              <span>Beranda</span>
            </NavLink>
            <NavLink to="/dashboard" className={({ isActive }) => `rw-bottom-item ${isActive ? 'active' : ''}`}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z" />
              </svg>
              <span>Laporan</span>
            </NavLink>
            <NavLink to="/lapor" className={({ isActive }) => `rw-bottom-item ${isActive ? 'active' : ''}`}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
        ...
        <NavLink to={user ? "/profil" : "/login"} className={({ isActive }) => `rw-bottom-item ${isActive ? 'active' : ''}`}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
    ...
    <div ref={wrapperRef} style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={dropdownMenu}>
          ...
          <button type="button" style={{ ...dropdownItem, color: '#A61C24' }} onClick={handleSignOut}>
            <LogoutIcon /> Keluar
          </button>
        </div>
      )}
