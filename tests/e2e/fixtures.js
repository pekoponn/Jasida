...
const session = {
  access_token: `eyJhbGciOiJIUzI1NiJ9.${payload}.local-signature`,
    ...
    await page.route('**/*', async (route) => {
      ...
      if (url.pathname === '/api/reset-password') {
        ...
        return route.fulfill({
          status: state.recoveryFailure ? 404 : 200,
          contentType: 'application/json',
          body: JSON.stringify(
            state.recoveryFailure ? { error: 'Email tidak terdaftar.' } : { success: true }
          ),
        });
      }
      ...
      if (url.origin === 'http://127.0.0.1:54321') {
        ...
        if (url.pathname.startsWith('/storage/v1/object/')) {
          ...
          return state.failUpload
            ? json(
                { message: 'Simulated storage failure', error: 'StorageFailure', statusCode: 500 },
                500
              )
        ...
        if (table === 'profiles') {
          ...
          if (state.profileDelay)
            await new Promise((resolve) => setTimeout(resolve, state.profileDelay));
      ...
      if (url.hostname === 'nominatim.openstreetmap.org')
        return json({ address: { road: 'Jalan Uji Lokal', city: 'Lokasi Simulasi' } });
