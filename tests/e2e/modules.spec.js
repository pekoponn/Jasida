...
test('location boundaries and conservative duplicate fallback', async ({ page, backend }) => {
  backend.reports = [
    { id: 'near', lat: -7.4478, lng: 112.7184, damage_type: 'pothole', created_at: new Date().toISOString() },
    { id: 'far', lat: -7.45, lng: 112.72, damage_type: 'pothole', created_at: new Date().toISOString() },
  ...
  const result = await page.evaluate(async () => {
    ...
    const candidates = await findSimilarReports({ ...sidoarjo, damageType: 'pothole', embedding: null });
    return {
      realAllowed: validateReportLocation(sidoarjo), realRejected: !!validateReportLocation(jakarta),
      trialAllowed: validateReportLocation(jakarta, true),
      invalid: [null, {lat:NaN,lng:112}, {lat:91,lng:0}, {lat:0,lng:181}, {lat:0,lng:Infinity}].every(p => !!validateReportLocation(p, true)),
      candidates, best: pickBestDuplicate(candidates), map: await listOpenReports()
    };
  });
