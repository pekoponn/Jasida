...
(async () => {
  ...
  const { uploadReportImage, addReportPhoto } = await import('/src/lib/reports.js');
  ...
  try {
    ...
    supabase.from = () => ({update: () => ({eq: async () => ({error: null})}), insert: async () => ({error: null})});
    ...
    assert(uploaded.length === 2, 'Unexpected number of storage writes');
  } finally {
