...
(async () => {
  const { prepareUploadPhoto, validateUploadPhoto, PHOTO_MAX_BYTES } = await import('/src/lib/imageUpload.js');
  ...
  for (const [type, width, height] of [
    ['image/png', 2400, 1800], ['image/jpeg', 2400, 1800],
    ['image/webp', 100, 80], ['image/png', 32, 4096]
  ]) {
    ...
    assert(Math.abs(decoded.width / decoded.height - width / height) < 0.03, 'Aspect ratio changed');
    ...
    results.push({input: type, inputBytes: input.size, outputBytes: output.size, width: decoded.width, height: decoded.height});
    decoded.close();
    validWebp = output;
  }

  await rejects(() => prepareUploadPhoto(new File([], 'empty.png', {type:'image/png'})), 'Empty accepted');
  await rejects(() => prepareUploadPhoto(new File(['broken'], 'bad.png', {type:'image/png'})), 'Corrupt accepted');
  await rejects(() => prepareUploadPhoto(new File(['<svg/>'], 'bad.svg', {type:'image/svg+xml'})), 'SVG accepted');
  await rejects(() => prepareUploadPhoto(new File([new Uint8Array(25 * 1024 * 1024 + 1)], 'huge.png', {type:'image/png'})), 'Huge input accepted');
  ...
  await rejects(() => validateUploadPhoto(new File(['fake'], 'bad.webp', {type:'image/webp'})), 'Invalid WebP header accepted');
  await rejects(() => validateUploadPhoto(new File([validWebp, new Uint8Array(100000 - validWebp.size)], 'boundary.webp', {type:'image/webp'})), '100 KB boundary accepted');

  const originalToBlob = HTMLCanvasElement.prototype.toBlob;
  try {
    HTMLCanvasElement.prototype.toBlob = function(callback) { callback(new Blob(['fallback'], {type:'image/png'})); };
  ...
  const { uploadReportImage, addReportPhoto, completeReportWithActuals } = await import('/src/lib/reports.js');
  ...
  const updates = [];
  try {
    supabase.storage.from = () => ({upload: async (path, file, options) => {
    ...
    supabase.from = () => ({
      update: (patch) => {
        updates.push(patch);
        return {eq: () => ({error: null, select: () => ({single: async () => ({data: patch, error: null})})})};
      },
    ...
    await rejects(() => uploadReportImage(new File(['bad'], 'bad.png', {type:'image/png'}), 'test-report'), 'Bad file reached upload');
    const converted = await convertToWebp(png);
    await validateUploadPhoto(converted);
    assert(await convertToWebp(converted) === converted, 'Compatibility helper recompressed a prepared photo');
    const materials = [{name: 'Test material', quantity: 2}];
    await completeReportWithActuals('test-report', {file: png, actualMaterials: 'Test', actualMaterialsJson: materials, actualCost: 10});
    assert(updates.at(-1).status === 'resolved' && updates.at(-1).actual_materials_json === materials, 'Completion lost status/material details');
    const updateCount = updates.length;
    await rejects(() => completeReportWithActuals('test-report', {file: new File(['bad'], 'bad.png', {type:'image/png'})}), 'Invalid resolution photo accepted');
    assert(updates.length === updateCount, 'Invalid photo changed report status');
    supabase.storage.from = () => ({upload: async () => ({error: new Error('Simulated storage failure')})});
    await rejects(() => completeReportWithActuals('test-report', {file: converted}), 'Failed upload marked report complete');
