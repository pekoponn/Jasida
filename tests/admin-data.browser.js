...
(async () => {
  ...
  try {
    supabase.from = (table) => {
      if (table === 'reports_with_coords')
        return {
          select: () => ({
            order: () => ({
              neq(column, value) {
                assert(
                  column === 'status' && value === 'pending_duplicate_review',
                  'Unexpected status filter'
                );
                return this;
              },
              data: [
                { id: 'test-report', user_id: 'test-user', lat: -7.45, lng: 112.7, status: 'open' },
              ],
              error: null,
            }),
          }),
      ...
      if (table === 'profiles')
        return {
          select: () => ({
            in: async () => ({
              data: [
                { id: 'test-user', username: 'Test', avatar_url: '/assets/avatar-1-oldhash.png' },
              ],
              error: null,
            }),
          }),
              ...
              data: [
                {
                  ...
                  bbox_area_pct: 12,
                  address: 'Test road',
                  estimated_materials_json: materials,
                },
              ],
    ...
    await startProgress('test-report', {
    ...
    assert(
      written.status === 'in_progress' && written.estimated_materials_json === materials,
      'Scheduling lost material details'
    );
    rejectDetails = true;
    let failed = false;
    try {
