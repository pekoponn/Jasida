...
(async () => {
  ...
  for (const [width, height] of [
    ...
    assert(
      ...
      'Invalid hazard score'
    );
    assert(result.imageWidth === width && result.imageHeight === height, 'Image dimensions lost');
    assert(
      result.detections.every(
        (d) => Number.isFinite(d.confidence) && d.confidence >= 0 && d.confidence <= 1
      ),
      'Invalid confidence'
    );
    samples.push({
  ...
  for (let a = 0; a < 8; a++)
    for (let i = 0; i < boxes.length; i++) data[a * boxes.length + i] = boxes[i][a];
  const detections = parseYoloOutput(
    { dims: [1, 8, boxes.length], data },
    { scale: 1, padX: 0, padY: 0, imageWidth: 100, imageHeight: 100 }
  );
  ...
  let rejected = false;
  try {
    parseYoloOutput({ dims: [1, 10, 1], data: new Float32Array(10) }, {});
  } catch {
    rejected = true;
  }
  ...
  for (const [score, expected] of [
    ...
    const result = computeHazardScore({
      detections: [{ damage_type: 'pothole', confidence: score / 70, bbox: [0, 0, 0, 0] }],
      imageWidth: 640,
      imageHeight: 640,
    });
  ...
  try {
    await embedImage(new File([], 'unused.webp'));
  } catch (err) {
    clipUnavailable = err.message.includes('belum tersedia');
  }
  ...
  return {
    ...
    samples,
    clipUnavailable,
    parserChecks: ['class-aware NMS', 'clipping', 'NaN rejection', 'model contract'],
  };
})();
