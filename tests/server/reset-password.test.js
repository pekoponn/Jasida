  ...
  const handler = createResetPasswordHandler({
    ...
    createAdmin: () => ({
        ...
        return found
          ? {
              data: {
                ...
                properties: { action_link: 'secret-recovery-link' },
              },
            }
      ...
      updateUserById: async (...args) => {
        ...
        return { error: failUpdate ? new Error('private details') : null };
      },
    }),
  });
  const res = {
...
test('disabled or unconfigured endpoint never calls Auth Admin', async () => {
  for (const env of [
    {},
    { ...enabled, ALLOW_EMAIL_ONLY_PASSWORD_RESET: 'false' },
    { ...enabled, SUPABASE_SERVICE_ROLE_KEY: '' },
  ]) {
...
test('matches normalized existing email and updates only that user without exposing tokens', async () => {
  ...
  assert.deepEqual(calls, [
    ['lookup', { type: 'recovery', email: 'person@example.test' }],
    ['update', 'user-1', { password: 'new-password' }],
  ]);
});
